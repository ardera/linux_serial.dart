...
void epollerEntry(Tuple2<SendPort, int> arg) {
  ...
  final epollEvents = ffi.allocate<epoll_event>(count: nEpollEvents);
  
  while (true) {
    final result = libc.epoll_wait(epollFd, epollEvents, nEpollEvents, -1);
    if (result < 0) {
      ...
      for (var i = 0; i < result; i++) {
        ...
        collected.add(epollEvent.userdata);
      }
...
class SynchronousComputer implements Computer {
  ...
  @override
  Future<void> turnOn({int workersCount = 2, bool areLogsEnabled = false}) {
    if (_isRunning) {
      ...
      _onTurnedOn = _computer
        .turnOn(workersCount: workersCount, areLogsEnabled: areLogsEnabled)
        .then((_) {
          print("is turned on");
          _isRunning = true;
        })
...
int executeWrite(Tuple4<int, int, int, int> param) {
  ...
  final write = ffi.Pointer
    .fromAddress(writeFuncAddr)
...
class PlatformInterface {
  PlatformInterface._construct(this.dylib, this.libc, this.serial, this.epollFd, this.onFdReady);
  ...
  factory PlatformInterface._private() {
    ...
    final serial = Serial(dylib);

    result = libc.epoll_create1(EPOLL_CLOEXEC);
    if (result < 0) {
  ...
  final _epollEventForFd = <int, ffi.Pointer<epoll_event>>{};
  final _computerForFd = <int, Computer>{};
  ...
  void makeRaw(int fd) {
    final ptr = ffi.allocate<termios>();
    final termiosState = ptr.ref;

    serial.tcgetattr(fd, ptr);
    ...
    serial.tcsetattr(fd, TCSANOW, ptr);
  ...
  void makeRawAndSetBaudrate(int fd, Baudrate baudrate) {
    ...
    result = serial.tcgetattr(fd, ptr);
    if (result < 0) {
      ffi.free(ptr);
    ...
    result = serial.cfsetispeed(ptr, baudrate.asLinuxValue);
    if (result < 0) {
      ffi.free(ptr);
      throw OSError("Could not set input speed for serial port. (cfsetispeed)");
    }

    result = serial.cfsetospeed(ptr, baudrate.asLinuxValue);
    if (result < 0) {
      ffi.free(ptr);
      throw OSError("Could not set output speed for serial port. (cfsetospeed)");
    }

    result = serial.tcsetattr(fd, TCSANOW, ptr);
    if (result < 0) {
      ffi.free(ptr);
  ...
  int open(String path, {Baudrate baudrate}) {
    ...
    final allocatedPath = ffi.Utf8.toUtf8(path);

    result = libc.open(allocatedPath.cast<ffi.Int8>(), O_RDWR | O_CLOEXEC | O_NONBLOCK);

    ffi.free(allocatedPath);
    ...
    final epollEvent = ffi.allocate<epoll_event>();

    epollEvent.ref.events = EPOLLIN | EPOLLPRI | EPOLLONESHOT;
    epollEvent.ref.userdata = fd;
  ...
  void setBaudrate(int fd, Baudrate baudrate) {
    ...
    serial.cfsetispeed(ptr, baudrate.asLinuxValue);
    serial.cfsetospeed(ptr, baudrate.asLinuxValue);
  ...
  Baudrate getBaudrate(int fd) {
    ...
    final inputSpeedAsLinuxValue = serial.cfgetispeed(ptr);
    final outputSpeedAsLinuxValue = serial.cfgetospeed(ptr);
  ...
  void flushInput(int fd) {
    final result = serial.tcflush(fd, TCIFLUSH);
    if (result != 0) {
  ...
  Future<int> write(int fd, ffi.Pointer buffer, int bufferSize) {
    ...
    return _computerForFd[fd].compute<Tuple4<int, int, int, int>, int>(
      executeWrite,
...
class SerialPorts {
  ...
  String _getDescriptionForMagicNumbers(int major, int minor) {
    if (major == 204) {
      if (minor <= 3) {
...
abstract class IOReader {
  factory IOReader(Stream<int> source, {Encoding encoding: utf8}) {
    return _IOReaderImpl(source, encoding: encoding);
  }

  @protected
  IOReader.construct({this.encoding});
  
  Encoding encoding;

  Future<List<int>> read(int bytes);

  /// Reads a string that has [stringLength] number of characters.
  Future<String> readStringWithLength(int stringLength);

  /// Read [numBytes] bytes and convert it into a string.
  Future<String> readStringWithNumBytes(int numBytes);
...
class _IOReaderImpl extends IOReader {
  _IOReaderImpl(this.source, {Encoding encoding})
  ...
  Future<String> readStringWithLength(int stringLength) async {
  ...
  Future<String> readStringWithNumBytes(int numBytes) {
    final encoding = this.encoding;
    return read(numBytes).then((bytes) => encoding.decode(bytes));
  }
  ...
  Future<void> close({bool immediate = false}) {
    return queue.cancel(immediate: false);
  }
}

class SerialPortHandle implements StringSink, IOReader {
  factory SerialPortHandle._private(int fd) {
    var readBuffer = ffi.allocate<ffi.Uint8>(count: _bufferSize);
    
    StreamSubscription<List<int>> fdReadySubscription;
    StreamController<List<int>> inputController;
    ...
    return SerialPortHandle._construct(
      fd,
      inputController,
      ffi.allocate<ffi.Uint8>(count: _bufferSize)
    );
  }
  ...
  Future<void> write(Object obj) {
    ...
    var remainingBytes = encoding.encode(string);
    final subTransmissions = <List<int>>[];
    do {
      subTransmissions.add(remainingBytes.sublist(0, min(remainingBytes.length, _bufferSize)));
      remainingBytes = remainingBytes.sublist(min(remainingBytes.length, _bufferSize));
    } while (remainingBytes.length > _bufferSize);

    return _writeLock.synchronized(() async {
      if (!_isOpen) return;

      for (final subTransmission in subTransmissions) {
        for (var i = 0; i < subTransmission.length; i++) {
          _writeBufferAsTypedList[i] = subTransmission[i];
        }
  ...
  Future<List<int>> read(int length) {
    ...
    sub = stream.listen(
      (bytes) {
        final toTake = min(bytes.length, length - result.length);
    ...
    sub = stream.listen(
      (bytes) {
        ...
        if (buffer.length >= stringLength) {
          final result = buffer.toString().substring(0, stringLength);
          sub.cancel();
          byteSink.close();
          stringSink.close();
          completer.complete(result);
  ...
  IOReader getNewSequentialReader() {
    return IOReader(stream.expand((element) => element), encoding: encoding);
  }
}